"""fprime.cli.fbuild: fbuild command line processing
...
from fprime.build.targets.enumerator import DesignateTargetAction
...
def add_special_targets(
    ...
    # It is not handled by argparse, but in fprime.cli.entry:validate()
    generate_parser.add_argument(
        "-D<VAR>[:<TYPE>]=<VALUE>",
        action="store_true",
...
def add_fbuild_parsers(
    ...
    wraps up all targets defined in the targets list in fprime.build.builder
