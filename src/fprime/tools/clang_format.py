...
from fprime.build.targets.target import ExecutableAction, TargetScope
...
class ClangFormatter(ExecutableAction):
    ...
    def execute(
        ...
        if not self.style_file.is_file():
            ...
            return 1
        # Backup files when --backup is requested, unless running only a --check
